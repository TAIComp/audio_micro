...
func (at *AudioTranscriber) shouldProcessAudio() bool {
	...
	// Only process audio if in FULL_LISTENING state
	shouldProcess := at.listeningState == FULL_LISTENING
...
func (at *AudioTranscriber) processResponses(responses []*speechpb.StreamingRecognizeResponse) {
	for _, resp := range responses {
		for _, result := range resp.Results {
			...
			// Skip processing if in INTERRUPT_ONLY state
	...
	go func() {
		for {
			...
			default:
				...
				// Send audio data to the queue only if shouldProcessAudio returns true
				if transcriber.shouldProcessAudio() {
					select {
